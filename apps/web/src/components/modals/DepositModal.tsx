...
export default function DepositModal() {
    ...
    if (isDepositing) {
      ...
      .then(data => {
        if (data.success && data.data) {
          ...
          if (intent.metadata?.intentUrl) setCustomUpiLink(intent.metadata.intentUrl);
        }
      })
      .catch(() => {});
    }
  ...
  const handleOpenUpiApp = (app: string) => {
    ...
    // Sandbox mode: Automatically mock the payment instead of freezing
    setLoading(true);
    toast(`Launching ${app}...`, { icon: '📲', duration: 1500 });
    
    setTimeout(() => {
      const mockUtr = 'SIM' + Math.floor(Math.random() * 1000000000);
      setLoading(false);
          ...
          {step === 'pay' ? (
            <>
              ...
              <div className="flex items-center justify-between text-[10px] text-gray-400">
                <span className="flex items-center gap-1 text-emerald-400 font-bold"><ShieldCheck size={13} /> Secured Wallet</span>
