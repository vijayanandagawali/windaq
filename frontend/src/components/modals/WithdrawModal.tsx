...
export default function WithdrawModal() {
  ...
  const handleWithdraw = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/v1/payments/withdraw-request', {
        method: 'POST',
      ...
      if (!data.success) {
        toast.error(data.error || 'Withdrawal failed. Please check details.');
        return;
      }

      const ok = withdraw(amount, upiId);
      if (ok) {
        toast.success(`🎉 Withdrawal of ₹${amount.toLocaleString('en-IN')} requested! Payout will be sent to ${upiId}.`, {
          duration: 4500
        });
